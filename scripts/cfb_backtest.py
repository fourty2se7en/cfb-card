...
if BOOKS.empty:
    ...
    ML = (BOOKS.groupby(["season", "week", "home_team", "away_team"])[["hml", "aml"]]
          .median().reset_index())
    Q = d.merge(ML, on=["season", "week", "home_team", "away_team"], how="inner")
    Q = Q[(Q.hml.abs() <= ML_SANE) & (Q.aml.abs() <= ML_SANE)]
    Q = Q[~((Q.hml < 0) & (Q.aml < 0))]
    if len(Q) < 200:
        ...
        ph_, pa_ = _imp(Q.hml.values), _imp(Q.aml.values)
        Q = Q.assign(mkt_home=np.clip(ph_ / (ph_ + pa_), 0.001, 0.999),
        ...
        for s in seasons[1:]:
            ...
            price = np.where(take_home, fut.hml.values, fut.aml.values)
            ...
            ml_rows.append(pd.DataFrame(dict(
                ...
                ret=np.where(won, _dec(price) - 1.0, -1.0),
...
try:
    ...
    if "R7" in globals() and len(R7) >= 100:
        roi = float(R7.ret.mean()) * 100
        se = float(R7.ret.std(ddof=1)) / np.sqrt(len(R7)) * 100
        w = int(R7.won.sum())
        record["moneyline"] = (w, int(len(R7) - w))
        notes["moneyline"] = "our calibrated winner against the price, walk-forward"
        returns["moneyline"] = dict(n=int(len(R7)), roi=round(roi, 2), se=round(se, 2),
                                    lo=round(roi - 1.96 * se, 2), hi=round(roi + 1.96 * se, 2),
                                    w=w, l=int(len(R7) - w))
        print(f"  moneyline           {w}-{int(len(R7)-w)}  return {roi:+.1f}% "

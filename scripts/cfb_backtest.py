...
def fit_venue(hist, ratings, hfa, weights):
...
def fit_as_of(season, week, ret_strength=0.0, ret_fade=0.0, ppa_weight=0.0,
    ...
    vfit = fit_venue(hist, out["rating"], out["hfa"], w) if venue else None
...
try:
    if not VEN:
        ...
        if len(VW) < 500:
            ...
            def venue_fit(season, week):
                k = (int(season), int(week))
                if k not in _vcache:
                    try:
                        _vcache[k] = fit_as_of(season, week, venue=True)
                    except Exception:
                        _vcache[k] = (None, None, None)
                return _vcache[k]

            # one pass: base prediction and the venue adjustment at full strength
            v_base = np.full(len(VW), np.nan)
            v_full = np.zeros(len(VW))
            coefs = []
            for (s_, w_), grp in VW.groupby(["season", "week"]):
                R_, hfa_, vf_ = venue_fit(int(s_), int(w_))
                if R_ is None:
                    continue
                v_base[grp.index.values] = (
                    R_.reindex(grp.home_team).values
                    - R_.reindex(grp.away_team).values
                    + np.where(grp.neutral.values, 0.0, hfa_))
                if vf_:
                    v_full[grp.index.values] = venue_adjust(grp, vf_, 1.0)
                    coefs.append(vf_["beta"])

            v_act = VW.margin.values
            v_mkt = -VW.close.values
            v_ok = np.isfinite(v_base) & np.isfinite(v_act)
            v_err = np.abs(v_act - v_base)
            print(f"  {len(VW):,} FBS vs FBS games with a closing line across "
                  f"{VW.season.nunique()} seasons")
            print(f"  fitting one home field for every ground: our MAE "
                  f"{v_err[v_ok].mean():.3f}, market MAE "
                  f"{np.abs(v_act - v_mkt)[v_ok].mean():.3f}, on {int(v_ok.sum()):,} games")

            fitted_coefs = {}
            if coefs:
                cm = np.mean(np.vstack(coefs), axis=0)
                cs = np.std(np.vstack(coefs), axis=0)
                fitted_coefs = {c: dict(mean=round(float(a), 3), sd=round(float(b), 3))
                                for c, a, b in zip(VENUE_COLS, cm, cs)}
                print(f"\n  what the ground is worth, averaged over "
                      f"{len(coefs)} refits (points):")
                units = {"altitude": "per 1,000 m of elevation",
                         "crowd": "per 10,000 seats",
                         "travel": "per 1,000 km the visitors travelled"}
                for c, a, b in zip(VENUE_COLS, cm, cs):
                    print(f"    {c:<9} {a:>+7.2f}  (sd {b:.2f} across refits)  {units[c]}")
                print(f"  spread of the adjustment across games: "
                      f"{v_full[v_ok].min():+.2f} to {v_full[v_ok].max():+.2f} points, "
                      f"sd {v_full[v_ok].std():.2f}")

            print(f"\n  {'weight':>7} {'gain':>8} {'95% interval':>18} "
                  f"{'better':>7} {'worse':>6}   verdict")
            v_grid = []
            for weight in VENUE_WEIGHT:
                p_ = v_base + weight * v_full
                m_ = v_ok & np.isfinite(p_)
                if m_.sum() < 400:
                    continue
                dd = v_err[m_] - np.abs(v_act[m_] - p_[m_])
                gain = float(dd.mean())
                se = float(dd.std(ddof=1) / np.sqrt(m_.sum()))
                lo, hi = gain - 1.96 * se, gain + 1.96 * se
                call = "HELPS" if lo > 0 else "hurts" if hi < 0 else "no evidence"
                v_grid.append(dict(weight=weight, n=int(m_.sum()),
                                   gain=round(gain, 4), se=round(se, 4),
                                   lo=round(lo, 4), hi=round(hi, 4), verdict=call,
                                   better=int((dd > 0).sum()),
                                   worse=int((dd < 0).sum())))
            ...
            v_best = max(v_grid, key=lambda r: r["gain"]) if v_grid else None
            v_split = []
            if v_best:
                print(f"\n    best: weight {v_best['weight']:.2f} -> "
                      f"{v_best['gain']:+.3f} points per game "
                      f"({v_best['lo']:+.3f} to {v_best['hi']:+.3f}), {v_best['verdict']}")
                # where the adjustment is largest is where it should show up
                p_ = v_base + v_best["weight"] * v_full
                big = np.abs(v_full) >= np.nanpercentile(np.abs(v_full[v_ok]), 75)
                for lbl, sel in (("adjustment small", v_ok & ~big),
                                 ("adjustment large", v_ok & big)):
                    m_ = sel & np.isfinite(p_)
                    if m_.sum() < 100:
                        continue
                    dd = v_err[m_] - np.abs(v_act[m_] - p_[m_])
                    se = float(dd.std(ddof=1) / np.sqrt(m_.sum()))
                    v_split.append(dict(span=lbl, n=int(m_.sum()),
                                        gain=round(float(dd.mean()), 4),
                                        lo=round(float(dd.mean()) - 1.96 * se, 4),
                                        hi=round(float(dd.mean()) + 1.96 * se, 4)))
                print(f"\n  a real effect should show up most where the adjustment "
                      f"is biggest:")
                print(f"  {'span':>17} {'games':>6} {'gain':>8} {'95% interval':>18}")
                for b in v_split:
                    iv = "({:+.3f} to {:+.3f})".format(b["lo"], b["hi"])
                    print(f"  {b['span']:>17} {b['n']:>6} {b['gain']:>+8.3f} {iv:>18}")
                venue_res = dict(grid=v_grid, best=v_best, by_size=v_split,
                                 coefficients=fitted_coefs,
                                 games=int(v_ok.sum()),
                                 adj_sd=round(float(v_full[v_ok].std()), 3),
                                 base_mae=round(float(v_err[v_ok].mean()), 3),
                                 market_mae=round(float(np.abs(v_act - v_mkt)[v_ok].mean()), 3),
                                 method="paired difference in absolute error, same "
                                        "games, walk-forward; venue slopes fitted "
                                        "from residuals after the fit, never as "
                                        "columns inside it")
except Exception as ex:
    import traceback

...
def wl(mask, win):
    s = win[mask]; w = int(s.sum()); return w, int(len(s) - w)
...
try:
    ...
    calibration["model_gap_early"] = fit_probit(p_spread[early], win1.values[early])
    calibration["model_gap_late"] = fit_probit(p_spread[~early], win1.values[~early])
    ...
    calibration["outright"] = fit_probit(p_home[ok], home_won[ok])
    ...
    if "flagged" in globals() and len(flagged):
        ...
        calibration["price_gap"] = fit_probit(pg_p, winp.values.astype(float))
    ...
    if "over" in globals() and len(t):
        ...
        # No model total exists in this test, so there is nothing to calibrate:
        # the strategy has a level and no slope, and the card must treat it so.
    ...
    sub("g5_vs_g5", d.g5 & (e >= 3), win1, "both sides Group of 5, 3+ point gap")
    ...
    payload = {
        ...
        "record": {k_: list(v) for k_, v in record.items()},
        "record_notes": notes,
        "headline": headline,

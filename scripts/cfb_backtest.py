...
# ---------------------------------------------------------------- fetching
# THREE THINGS THIS SOLVES, all found when a run died half way through 2020.
#
# 1. A CALL THAT FAILS NOW STOPS THE RUN. It used to return None after four
#    tries, and only /games checked for that. Talent, returning production,
#    per-play efficiency, lines and venues all carried on with nothing. A run
#    that got throttled part way could therefore finish GREEN and write a
#    record built on missing data, and every grade on the card cites that
#    record. Same family as everything in 4.3. An EMPTY answer is still fine
#    and still handled by the callers: empty is data, a failure is not.
#
# 2. FINISHED SEASONS ARE FETCHED ONCE. 2016 to last season will never change
#    again, so they are cached to scripts/data/backtest and committed. A run
#    used to cost 51 calls out of a 1,000-a-month allowance, of which the card
#    already spends about 720. It now costs about 5 after the first build.
#
# 3. CALLS ARE PACED, AND A "SLOW DOWN" IS OBEYED. Fifty-one calls fired back
#    to back is what triggered the throttle; the card, at eight, never does.
#    Retry-After is honoured when the server sends one instead of guessed at.
CACHE_DIR = os.path.join(BASE, "data", "backtest")
os.makedirs(CACHE_DIR, exist_ok=True)
with open(os.path.join(CACHE_DIR, "README.md"), "w") as _f:
    _f.write(
        ...
        "the calls.\n")
MIN_GAP = 1.2          # seconds between live calls
TRIES = 6
_last_call = [0.0]


def _season_now():
    """The season being played. CFBD seasons run August to January."""
    t = dt.date.today()
    return t.year if t.month >= 8 else t.year - 1


CUR_SEASON = int(os.environ.get("CFB_SEASON", _season_now()))


def _cache_file(path, params):
    key = path.strip("/").replace("/", "_")
    extra = "_".join(f"{k}-{v}" for k, v in sorted(params.items()) if k != "year")
    if extra:
        key += "_" + hashlib.sha1(extra.encode()).hexdigest()[:8]
    return os.path.join(CACHE_DIR, f"{key}_{params.get('year', 'all')}.json.gz")


def get(path, season=None, **params):
    """One CFBD call. Cached when the season it asks about is finished.

    season=None means the answer is not tied to a finished season, so it is
    always fetched live. Anything that fails after TRIES attempts EXITS: this
    script's whole job is to describe the live model, and describing it from
    half the data is worse than not running.
    """
    cacheable = season is not None and int(season) < CUR_SEASON
    fp = _cache_file(path, params) if cacheable else None
    if fp and os.path.exists(fp):
        try:
            with gzip.open(fp, "rt", encoding="utf-8") as f:
                return json.load(f)
        except Exception as e:
            print(f"  cache unreadable at {os.path.basename(fp)} ({e}), refetching")
        ...
        if r.status_code == 200:
            ...
            if fp:
                try:
                    ...
                    print(f"  could not cache {os.path.basename(fp)}: {e}")
            return data
        if r.status_code in (401, 403):
            sys.exit(f"ERROR: CFBD rejected the key on {path} (HTTP {r.status_code}).")
        if r.status_code == 429:
            # Obey the server's own number where it sends one.
    ...
    sys.exit(
        f"ERROR: {path} (year {params.get('year', 'n/a')}) did not come back after "
        f"{TRIES} attempts. STOPPING rather than measuring the model on partial data "
        f"and writing that record over a good one.\n"
        f"       The free CFBD tier allows 1,000 calls a month and the card already "
        f"spends about 720 of them. Finished seasons are cached under "
        f"scripts/data/backtest, so re-running this after a successful build costs "
        f"only the current season."
    )
...
for yr in SEASONS:
    g = get("/games", season=yr, year=yr, seasonType="regular")
    if not g:
        sys.exit(f"ERROR: no games came back for {yr}")
    d = pd.json_normalize(g); d["season"] = yr
    games.append(d)
    t = get("/talent", season=yr, year=yr)
    if t:
    ...
    rp = get("/player/returning", season=yr, year=yr)
    if rp:
    ...
    pp = get("/ppa/games", season=yr, year=yr, seasonType="regular",
...
# Stadium metadata, for the per-venue home field test. One home-field number is
# used for every ground today, which cannot be right: Laramie sits at 2,200
# metres and Miami at sea level, and a team flying 2,000 miles is not in the
# same state as one driving across town.
VEN = {}
_v = get("/venues")
if _v:
    _vd = pd.json_normalize(_v)
    if "id" in _vd.columns:
        for c in ("elevation", "capacity", "latitude", "longitude"):
            _vd[c] = pd.to_numeric(_vd.get(c), errors="coerce")
        VEN = {int(r.id): (r.elevation, r.capacity, r.latitude, r.longitude)
               for r in _vd.itertuples() if np.isfinite(r.id)}
print(f"venues with metadata: {len(VEN):,}")

G["venue_id"] = pd.to_numeric(G.get("venueId"), errors="coerce")
...
rows, bookrows = [], []
for yr in TEST:
    L = get("/lines", season=yr, year=yr, seasonType="regular")
    if not L:
...
def haversine_km(lat1, lon1, lat2, lon2):
    """Great-circle distance. Not driving distance, but the two rank the same."""
    r = 6371.0
...
def home_venues(rows):
    """Each team's usual home ground, from its own non-neutral home games.

    Taken from history only, never from the games being predicted.
    """
    hm = rows[~rows.neutral.values]
    out = {}
    for t, grp in hm.groupby("home_team"):
        v = grp.venue_id.dropna()
        if len(v):
            out[t] = int(v.mode().iloc[0])
    return out


VENUE_COLS = ("altitude", "crowd", "travel")


def venue_features(rows, hv):
    """Three measurable things a single home-field number cannot express.

    altitude  height of the ground, in km
    crowd     seats, in ten thousands
    travel    how far the visitors came, in thousands of km

    All zero on neutral sites, where there is no home field to adjust. A game
    whose stadium or opponent cannot be located gets zeros too, which after
    centring means "treat as average" rather than "guess".
    """
    n = len(rows)
    f = np.zeros((n, 3))
    vid = rows.venue_id.values
    neut = rows.neutral.values.astype(bool)
    away = rows.away_team.values
    for i in range(n):
        if neut[i]:
            continue
        v = VEN.get(int(vid[i])) if np.isfinite(vid[i]) else None
        if v is None:
            continue
        elev, cap, lat, lon = v
        if np.isfinite(elev):
        ...
        if np.isfinite(cap) and cap > 0:
            f[i, 1] = cap / 10000.0
        av = VEN.get(hv.get(away[i], -1))
        if av is not None and np.isfinite(lat) and np.isfinite(lon) \
                and np.isfinite(av[2]) and np.isfinite(av[3]):
            f[i, 2] = haversine_km(av[2], av[3], lat, lon) / 1000.0
    return f


def fit_venue(hist, ratings, hfa, weights, flat=False):
    """How much of what the fit MISSED is explained by the ground itself.

    Measured from residuals AFTER the fit, deliberately, and not added as
    columns inside it. Section 4.3a is the whole reason: the division gap went
    in as a column, ridge split it between the team dummies arbitrarily, the
    sign inverted, and FCS teams floated to the top of the table. Putting
    altitude in as a column invites the same failure, because a team's ground
    never changes, so "this stadium is high" and "this team is good" are hard
    for a penalised fit to tell apart.

    Fitted on this history only. Returns the slopes and the centres they were
    measured against, so a prediction is adjusted on the same scale.

    flat controls WHICH history counts. The model's own weights halve every few
    weeks, because a team's form goes stale. A mountain does not. Altitude,
    stadium size and the distance between two cities are the same in 2019 as
    they are today, so down-weighting an old game when measuring them throws
    away sample for nothing. Both are measured rather than assumed, because
    "obviously" is how the constants in section 6 went wrong.
    """
    m = (~hist.neutral.values.astype(bool))
    if int(m.sum()) < 400:
        return None
    hv = home_venues(hist)
    f = venue_features(hist[m], hv)
    w = np.asarray(weights, dtype=float)[m]
    pred = (ratings.reindex(hist.home_team[m]).values
            - ratings.reindex(hist.away_team[m]).values + hfa)
    resid = hist.margin.values[m] - pred
    ok = np.isfinite(resid) & np.isfinite(f).all(axis=1)
    if int(ok.sum()) < 400:
        return None
    f, w, resid = f[ok], (np.ones(int(ok.sum())) if flat else w[ok]), resid[ok]
    # centre so the adjustment averages to zero and the base home field, which
    # is already fitted, is not quietly moved by this
    mu = (f * w[:, None]).sum(axis=0) / max(w.sum(), 1e-9)
    fc = f - mu
    sw = np.sqrt(w)
    beta, *_ = np.linalg.lstsq(fc * sw[:, None], resid * sw, rcond=None)
    return dict(beta=beta, mu=mu, hv=hv, n=int(ok.sum()))


def venue_adjust(rows, vfit, weight=1.0):
    """Extra home-field points for each game. Zero when the switch is off."""
    if not vfit or weight == 0:
        return np.zeros(len(rows))
    raw = venue_features(rows, vfit["hv"])
    adj = (raw - vfit["mu"]) @ vfit["beta"]
    # a neutral or unlocatable game must stay at exactly zero, not at minus the
    # centre, so put those rows back
    adj[~raw.any(axis=1)] = 0.0
    return weight * adj
...
def fit_as_of(season, week, ret_strength=0.0, ret_fade=0.0, ppa_weight=0.0,
    ...
    out = FIT.fit_ratings(hist, TEAMS, TIDX, talent_prior(season), weights=w,
                          ...
                          target=_target)
    # The per-venue switch. None means off, and then nothing downstream adds
    # anything at all, so the model is the one the card publishes.
    vfit = (fit_venue(hist, out["rating"], out["hfa"], w, flat=(venue == "flat"))
            if venue else None)
    return out["rating"], out["hfa"], vfit
        ...
        if len(WK) < 300:
            ...
            def predict(strength, fade):
                ...
                for (s_, w_), grp in WK.groupby(["season", "week"]):
                    R_, hfa_, _vf = fitted(int(s_), int(w_), strength, fade)
                    if R_ is None:
                        continue
...
# ============================================================================
# TEST 10 - PER-VENUE HOME FIELD, PAIRED
#
# The model uses one home-field number for every ground in the country. That
# cannot be right on its face: Laramie is 2,200 metres up, Miami is at sea
# level, 100,000 people is not 12,000 people, and a team flying across three
# time zones is not a team that drove across town.
#
# The question is whether any of that is worth points on top of what the team
# ratings already capture.
#
# Measured from residuals AFTER the fit, never as columns inside it. That is
# 4.3a, and it applies here more sharply than it did there: a team's ground
# never changes, so "this stadium is high" and "this team is good" are close to
# the same column as far as a penalised regression is concerned. Fitting it
# afterwards on what the model MISSED cannot destabilise a single rating.
#
# One useful consequence: the ratings do not depend on the weight, so the whole
# grid reuses one set of fits and this test is nearly free.
# ============================================================================
print("\n" + "=" * 78)
print("TEST 10 - PER-VENUE HOME FIELD, PAIRED")
print("=" * 78)

VENUE_WEIGHT = (0.25, 0.5, 0.75, 1.0)   # 0 is the current model and is the baseline

venue_res = {}
try:
    if not VEN:
        print("  no venue metadata came back, so this cannot be tested")
    else:
        VW = LINES.merge(G, on=["season", "week", "home_team", "away_team"], how="inner")
        VW = VW[np.array([is_fbs(h) and is_fbs(a)
                          for h, a in zip(VW.home_team, VW.away_team)])]
        VW = VW[np.isfinite(VW.close.values)].reset_index(drop=True)
        if len(VW) < 500:
            print(f"  only {len(VW)} graded games, too few")
        else:
            _vcache = {}
            v_act = VW.margin.values
            v_mkt = -VW.close.values
            units = {"altitude": "per 1,000 m of elevation",
                     "crowd": "per 10,000 seats",
                     "travel": "per 1,000 km the visitors travelled"}
            print(f"  {len(VW):,} FBS vs FBS games with a closing line across "
                  f"{VW.season.nunique()} seasons")

            def venue_fit(season, week, mode):
                k = (int(season), int(week), mode)
                if k not in _vcache:
                    try:
                        _vcache[k] = fit_as_of(season, week, venue=mode)
                    except Exception:
                        _vcache[k] = (None, None, None)
                return _vcache[k]

            def run_venue(mode, label):
                """One pass gives the base prediction and the full adjustment.

                The ratings do not depend on the weight, so the whole grid is
                one scaling of the same numbers and costs almost nothing.
                """
                base = np.full(len(VW), np.nan)
                full = np.zeros(len(VW))
                coefs = []
                for (s_, w_), grp in VW.groupby(["season", "week"]):
                    R_, hfa_, vf_ = venue_fit(int(s_), int(w_), mode)
                    if R_ is None:
                        continue
                    base[grp.index.values] = (
                        R_.reindex(grp.home_team).values
                        - R_.reindex(grp.away_team).values
                        + np.where(grp.neutral.values, 0.0, hfa_))
                    if vf_:
                        full[grp.index.values] = venue_adjust(grp, vf_, 1.0)
                        coefs.append(vf_["beta"])
                ok = np.isfinite(base) & np.isfinite(v_act)
                err = np.abs(v_act - base)
                print(f"\n  --- {label} ---")
                print(f"  one home field for every ground: our MAE "
                      f"{err[ok].mean():.3f}, market MAE "
                      f"{np.abs(v_act - v_mkt)[ok].mean():.3f}, on {int(ok.sum()):,} games")
                fc = {}
                if coefs:
                    st = np.vstack(coefs)
                    cm, cs = st.mean(axis=0), st.std(axis=0)
                    fc = {c: dict(mean=round(float(a), 3), sd=round(float(b), 3))
                          for c, a, b in zip(VENUE_COLS, cm, cs)}
                    print(f"  what the ground is worth, averaged over {len(coefs)} refits:")
                    for c, a, b in zip(VENUE_COLS, cm, cs):
                        steady = "steady" if abs(b) < 0.4 * max(abs(a), 1e-9) else "UNSTABLE"
                        print(f"    {c:<9} {a:>+7.2f} points  (sd {b:.2f} across "
                              f"refits, {steady})  {units[c]}")
                    print(f"  adjustment across games: {full[ok].min():+.2f} to "
                          f"{full[ok].max():+.2f} points, sd {full[ok].std():.2f}")
                print(f"  {'weight':>7} {'gain':>8} {'95% interval':>18} "
                      f"{'better':>7} {'worse':>6}   verdict")
                grid = []
                for weight in VENUE_WEIGHT:
                    p_ = base + weight * full
                    m_ = ok & np.isfinite(p_)
                    if m_.sum() < 400:
                        continue
                    dd = err[m_] - np.abs(v_act[m_] - p_[m_])
                    gain = float(dd.mean())
                    se = float(dd.std(ddof=1) / np.sqrt(m_.sum()))
                    lo, hi = gain - 1.96 * se, gain + 1.96 * se
                    call = "HELPS" if lo > 0 else "hurts" if hi < 0 else "no evidence"
                    grid.append(dict(weight=weight, n=int(m_.sum()),
                                     gain=round(gain, 4), se=round(se, 4),
                                     lo=round(lo, 4), hi=round(hi, 4), verdict=call,
                                     better=int((dd > 0).sum()),
                                     worse=int((dd < 0).sum())))
                    print(f"  {weight:>7.2f} {gain:>+8.3f} "
                          f"{f'({lo:+.3f} to {hi:+.3f})':>18} "
                          f"{int((dd > 0).sum()):>7} {int((dd < 0).sum()):>6}   {call}")
                best = max(grid, key=lambda r: r["gain"]) if grid else None
                split = []
                if best:
                    print(f"    best: weight {best['weight']:.2f} -> "
                          f"{best['gain']:+.3f} ({best['lo']:+.3f} to "
                          f"{best['hi']:+.3f}), {best['verdict']}")
                    # A real effect must show up MOST where the adjustment is
                    # biggest. If instead the damage is concentrated there, the
                    # slopes are noise being applied with confidence.
                    p_ = base + best["weight"] * full
                    big = np.abs(full) >= np.nanpercentile(np.abs(full[ok]), 75)
                    for lbl, sel in (("adjustment small", ok & ~big),
                                     ("adjustment large", ok & big)):
                        m_ = sel & np.isfinite(p_)
                        if m_.sum() < 100:
                            continue
                        dd = err[m_] - np.abs(v_act[m_] - p_[m_])
                        se = float(dd.std(ddof=1) / np.sqrt(m_.sum()))
                        split.append(dict(span=lbl, n=int(m_.sum()),
                                          gain=round(float(dd.mean()), 4),
                                          lo=round(float(dd.mean()) - 1.96 * se, 4),
                                          hi=round(float(dd.mean()) + 1.96 * se, 4)))
                    print(f"  {'span':>17} {'games':>6} {'gain':>8} {'95% interval':>18}")
                    for b in split:
                        iv = "({:+.3f} to {:+.3f})".format(b["lo"], b["hi"])
                        print(f"  {b['span']:>17} {b['n']:>6} {b['gain']:>+8.3f} {iv:>18}")
                return dict(grid=grid, best=best, by_size=split, coefficients=fc,
                            refits=len(coefs), games=int(ok.sum()),
                            adj_sd=round(float(full[ok].std()), 3),
                            adj_max=round(float(np.abs(full[ok]).max()), 3),
                            base_mae=round(float(err[ok].mean()), 3),
                            market_mae=round(float(np.abs(v_act - v_mkt)[ok].mean()), 3))

            venue_res = {
                "recency_weighted": run_venue("recency",
                    "slopes fitted with the model's recency weights"),
                "flat_weighted": run_venue("flat",
                    "slopes fitted on all history equally, since geography does not decay"),
                "method": "paired difference in absolute error, same games, "
                          "walk-forward; venue slopes fitted from residuals after "
                          "the fit, never as columns inside it",
            }
            _r, _f = venue_res["recency_weighted"], venue_res["flat_weighted"]
            print(f"\n  the two ways of measuring the slopes, side by side:")
            print(f"    recency weighted  best {_r['best']['gain']:+.3f}  "
                  f"adjustment sd {_r['adj_sd']:.2f}  largest {_r['adj_max']:.2f}")
            print(f"    all history equal best {_f['best']['gain']:+.3f}  "
                  f"adjustment sd {_f['adj_sd']:.2f}  largest {_f['adj_max']:.2f}")
except Exception as ex:
    import traceback
    print(f"  SKIPPED venue test: {ex}")
    traceback.print_exc(file=sys.stdout)

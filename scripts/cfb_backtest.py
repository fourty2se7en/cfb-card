...
TURN_FADE = (4.0, 8.0, 14.0)      # 14 is effectively "never fades in-season"
TURN_WEEKS = (1, 2, 3, 4)
        ...
        if len(WK) < 300:
            ...
            for fade in TURN_FADE:
                for strength in TURN_STRENGTH:
                    ...
                    d = base_err[m_] - np.abs(act[m_] - p[m_])
                    gain = float(d.mean())
                    se = float(d.std(ddof=1) / np.sqrt(m_.sum()))
            ...
            if best:
                ...
                for w_ in TURN_WEEKS:
                    ...
                    byweek.append(dict(week=int(w_), n=int(m_.sum()),
                                       ...
                                       lo=round(float(d.mean()) - 1.96 * se, 4),
                                       hi=round(float(d.mean()) + 1.96 * se, 4)))

...
try:
    ...
    if "R7" in globals() and len(R7) >= 100:
        ...
        for b in cf_buckets:
            if b["interval"][1] < 0:
                ...
                break
        if measured is None:
            note = ("no bucket lost with its whole interval below zero, so there is "
                    "no measured floor and the chosen one stands")
        else:
    ...
    if sensitivity:
        verdicts = {v["verdict"] for v in sensitivity.values()}
        ...
        if len(verdicts) == 1:
            print(f"    -> same verdict on every window ({verdicts.pop()}), "
                  f"so the start year is not doing the work")
        else:
            print(f"    -> the verdict CHANGES with the window ({', '.join(sorted(verdicts))}). "
                  f"The start year is doing some of the work and this result is not safe to quote.")

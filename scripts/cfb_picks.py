...
# to prevent.
# The floor is now MEASURED where the evidence supports one. cfb_backtest.py
# buckets moneyline bets by calibrated probability and asks whether each bucket
# made money. Where the bottom buckets lost with their whole 95% interval below
# zero, the top of that run is the floor and it is read from here. Where no
# bucket did, there is no measured floor and the chosen 40 stands, which is the
# honest fallback: absence of evidence for a floor is not evidence against one.
#
# .get, not [], throughout, because model_state.py falls back to values compiled
# into itself when the JSON is missing or broken, and that copy predates both
# keys. A gate that raises KeyError would cost the whole card, which is what the
...
# week instead of only when the backtest is re-run. Filled by cfb_card.py once it
# has read picks.csv; empty here so the module still works on its own.
# {strategy: {"w":, "l":, "p":, "rets":[...]}}
LIVE = {}
...
def _pool_returns(bt, rets):
    ...
    sample standard deviation, which is what lets the two samples be pooled
    ...
    n = n0 + n1
    m = (n0 * m0 + n1 * m1) / n
    # between-group term, so a live sample that disagrees widens the interval
    between = n0 * (m0 - m) ** 2 + n1 * (m1 - m) ** 2
...
def grade_market(p_win, edge_pts, strategy, price=None, conf_strategy=None):
    ...
    if 100.0 * p_cal < CONF_FLOOR:
        ...
        if new != letter:
            ...
            capped_by = (f"only {100 * p_cal:.0f}% likely, below the {kind} {CONF_FLOOR:.0f}% "
                         f"floor, so the price cannot lift it")
        letter = new

...
WHAT HAD TO CHANGE, AND WHY
    ...
    unchecked. Every one of these was measured on this project's own data:

      our uncertainty about the true line   7.0   NFL uses 3.0
      margin around the true line          15.0   NFL uses 13.2
      our uncertainty about the total       4.1
      points around the true total         13.3   NFL uses 10.4

    Our line uncertainty is more than double the NFL's. That is the honest
    number: the standard deviation of our number minus the market's over 154
    games is 7.0.

    Margin scatter grows with the total, measured over 762 FBS games from 2025,
    so the draw widens for shootouts and tightens for rock fights, which is the
    same adjustment the NFL card makes.
...
THE FOURTH GATE, WHICH THE NFL CARD DOES NOT NEED
    ...
    is trustworthy. In college it is not, and that is measured, not suspected:
      ...
      the model's median simulated cover probability is 60.2%
      games it puts at 3 to 6 points of edge actually covered 50.7% (n=880)
      games it puts at 6 to 10 points actually covered 49.1% (n=466)

    A 60% simulated cover is a coin flip in reality. Run the NFL scheme on this
    slate with no fourth gate and it grades 115 of 154 spreads an A, on a
    strategy that backtested at 47.4% in weeks 1 to 4. Moving the scale
    endpoints only changes how many A grades get printed; it cannot make them
    mean anything.
    ...
    This gate lifts on its own. Update RECORD from a cfb-backtest run and every
    grade recalculates.
"""
import numpy as np
...
# --- simulation constants, all measured on college data ---
LINE_SD = 7.0          # our uncertainty about the true spread. NFL uses 3.0
MARGIN_SD = 15.0       # margin around the true line at a total of 52. NFL 13.2
TOTAL_LINE_SD = 4.1    # our uncertainty about the true total
TOTAL_SD = 13.3        # points around the true total. NFL uses 10.4
# |residual| = 0.0541 * total + 7.35, fitted on 762 FBS games, normalised at 52
TOTAL_TILT = (0.0541, 7.35, 10.16)

# --- the NFL scoring scale, unchanged ---
P_FLOOR, P_CEIL = 0.40, 0.60
VALUE_CEIL = 8.0
WIN_POINTS, VALUE_POINTS = 70.0, 30.0
...
RECORD = {
    ...
    "total_over":      (1418, 1391),  # the market total
}
...
def price_notes_to_picks(game, price_notes, threshold=3.0):

...
def dedupe_books(books):
    ...
    for b in books:
        ...
        score = sum(1 for v in b.values() if v is not None)
...
for _, g in week_games.iterrows():
    ...
    books = dedupe_books(lr["_books"] if lr is not None else [])
...
for _, g in CUR.dropna(subset=["home_points", "away_points"]).iterrows():
    ...
    for _, x in LINES[LINES.week == g.week].iterrows():
        if str(x.awayTeam) == away and str(x.homeTeam) == home:
            lr = x; break
    mkt = med([b.get("spread") for b in dedupe_books(lr["_books"] if lr is not None else [])])
    mm = model_margin(home, away, bool(g.neutral))
...
SENS_VERDICTS = sorted({str(v.get("verdict", "")) for v in SENS.values()})
SENS_STABLE = len(SENS_VERDICTS) == 1
...
def sens_html():
    ...
    if SENS_STABLE:
        ...
        tail = ("<p><b>The verdict changes with the window (" + esc(", ".join(SENS_VERDICTS))
                + ").</b> The start year is doing some of the work, so the pooled figure "
                  "above is not safe to quote on its own.</p>")

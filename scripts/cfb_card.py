...
for _c in NLCOLS:
    if _c not in nlog.columns:
        nlog[_c] = None
# Same key spelling on both sides of the comparison, for the reason pkey exists.
_nseen = {pkey(r.season, r.week, r.away, r.home) for r in nlog.itertuples()}
_nnew = []
for r in rows:
    ...
    _nnew.append(dict(
        ...
        qb_out=str(_lg.get("qb_out", "") or ""),
        starters_out=_lg.get("starters_out"),
        confidence=str(_lg.get("confidence", "") or ""),
...
.pk-a {{ background:var(--abg); color:var(--a); }}
.pk-b {{ background:var(--bbg); color:var(--b); }}
.pk-c {{ background:var(--cbg); color:var(--c); }}
.pk-d {{ background:var(--dbg); color:var(--d); }}

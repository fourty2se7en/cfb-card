...
for r in rows:
    ...
    if r["kick"]:
        try:
            ...
            pass
    seed = abs(hash((r["away"], r["home"], WEEK))) % (2 ** 31)
...
cannot, and that is measured rather than suspected. The model's median simulated cover
probability is 60.2%. Games it puts at 3 to 6 points of edge actually covered 50.7% over 880
backtested games; at 6 to 10 points, 49.1% over 466. A 60% simulated cover is a coin flip in
reality.</p>
<p>Run this scheme with no fourth gate and it grades 115 of 154 spreads an A, on a strategy
that backtested at 47.4% in weeks 1 to 4. Moving the scale endpoints only changes how many A
grades get printed; it cannot make them mean anything. So: <b>a market whose strategy
backtested with its whole 95% interval below break-even cannot grade above D.</b> A star on
a grade means it was capped, and the raw score is still shown on the game so the cap is
visible rather than silent.</p>
<p>The gate lifts on its own. Re-run the cfb-backtest workflow, update the record, and every
grade recalculates.</p>

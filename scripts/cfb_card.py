...
ML_SANE = 5000        # Bovada and others post -100000 as a placeholder meaning "no
                      # price". Taking a median across books with that in it produced
                      # "Coastal Carolina -49550" on a live page. A real market never
                      # has both sides negative, and never gets past about -5000.
# quotes dropped for this reason are counted below and surfaced in the panel


def sane_pair(hml, aml):
    """True only if this looks like a real two-sided market."""
...
for _, g in week_games.iterrows():
    ...
    global_bad_ml += sum(1 for b in books
                  if b.get("homeMoneyline") is not None and b.get("awayMoneyline") is not None
    ...
    for b in books:
        ...
        if s is None or not sane_pair(hml, aml):
            continue
...
def check():
    ...
    if global_bad_ml:
        out.append(("placeholder moneylines",
                    f"{global_bad_ml} book quotes on this week's card carried an unusable "
                    f"moneyline, such as -100000 meaning no price. They were dropped before "
                    f"the consensus was taken, so the numbers above are from real quotes only."))
    if not rows:

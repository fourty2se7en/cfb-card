...
def summary_table():
    ...
    for r in rows:
        ...
        body.append(
            ...
            f'<td class="game">{esc(r["away"])} <span class="num">@</span> '
            f'{esc(r["home"])}</td>'
...
def compare_table(r):
    ...
    return (f'<table class="cmp"><thead><tr><th></th>'
            f'<th class="ca">{esc(a)}</th><th class="ch">{esc(h)}</th>'
...
def preview_rows(r):
    ...
    def line(t):
        rt = _tv(t, "rating")
        return (f'<span class="pvT">{esc(t)}</span>'
...
def game_card(r):
    ...
    return (f'<details class="gc" data-day="{esc(day)}" data-gid="{esc(gid)}"><summary>'
            ...
            f'<span><span class="gcH">{esc(r["away"])}'
            f'<span class="vs">at</span>{esc(r["home"])}</span>'
...
.t-d {{ background:var(--nbg); color:var(--n); }}
...
.pk-d {{ color:var(--n); font-weight:700; }}

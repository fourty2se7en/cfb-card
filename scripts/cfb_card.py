...
def check():
    ...
    if not _fresh:
        ...
        _overdue, _now = [], dt.datetime.now(dt.timezone.utc)
        for _n, _e in sorted(_fresh.items()):
            ...
            if not _at or not _max:
                continue
            try:
                _w = dt.datetime.fromisoformat(str(_at))
                if _w.tzinfo is None:
                    _w = _w.replace(tzinfo=dt.timezone.utc)
                _age = (_now - _w).total_seconds() / 86400.0
            except Exception:
                continue
            if _age > float(_max):
